<template>
	<view class="consult-chat">
		...
		<view class="chat-banner" style="background:linear-gradient(135deg,#D4E7F7,#A4C7E8)">
			...
			<text class="banner-title">狗狗疾病咨询</text>
			<text class="banner-desc">专业兽医在线解答，快速诊断狗狗健康问题</text>
		</view>
		...
		<view class="symptom-section">
			<view class="section-title">请描述您狗狗的症状</view>
			...
			<view class="symptom-input-wrap">
				<textarea class="symptom-input" v-model="symptomDesc" placeholder="详细描述狗狗的症状..." />
			</view>
...
<script setup>
	...
	const selectDisease = (item) => {
		uni.navigateTo({
			url: `/pages/newbie/consult/dog-doctor/disease/detail/detail?name=${encodeURIComponent(item.name)}&icon=${encodeURIComponent(item.icon)}&desc=${encodeURIComponent(item.desc)}`
		})
	}
...
<style lang="scss" scoped>
	...
	.doctor-avatar {
		...
		height: 60rpx;
		border-radius: 50%;
		background: linear-gradient(135deg, #D4E7F7, #A4C7E8);
		display: flex;
		align-items: center;
		justify-content: center;
		font-size: 28rpx;
